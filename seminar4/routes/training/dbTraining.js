...
router.get('/:gender', async(req, res) => {
    const womanQuery = 'SELECT * FROM membership WHERE gender = 1';
    const womanResult = await db.queryParam_None(womanQuery);
    if (!womanResult) {
        res.status(200).send(defaultRes.successFalse(statusCode.DB_ERROR, resMessage.MEMBERSHIP_SELECT_FAIL));
    } else {
        res.status(200).send(defaultRes.successTrue(statusCode.OK, resMessage.MEMBERSHIP_SELECT_SUCCESS, womanResult));
    }
});
